...
- **Tooltip** - 툴팁 (top, bottom, left, right placement, 다크모드 지원, CSS 변수 사용)
- **Popup** - 팝업 (기본: Swiper 캐러셀 지원, lazy loading / 시트: 드래그로 닫기 / 풀스크린: showHeaderClose, showBottomClose props로 3가지 타입 지원, 모바일 주소창 대응을 위한 `100dvh` 및 `env(safe-area-inset-bottom)` 사용)
...
#### 드롭다운 & 피커
- **Dropdown** - 드롭다운
- **DatePicker** - 날짜 선택 (단일/범위/다중 선택, 닫기 버튼, 자동 닫기 제거, 다크모드 지원, range 선택 시 다크모드 색상 최적화)
...
- ✅ 수신자 선택 및 제거 기능
- ✅ 카드 썸네일 선택 (가로 스크롤, 선택된 카드 가운데 정렬, 화살표 표시)
- ✅ 메시지 입력 화면 (일러스트와 텍스트 영역 통합, 힌트 텍스트 오버레이)
- ✅ 발송 옵션 선택 (즉시 발송/예약 발송, 라디오 버튼 가로 배치)
- ✅ 날짜/시간 선택기 (DayPicker, 커스텀 시간 선택 UI, 중앙 고정 팝업, 딤드 배경)
- ✅ 미리 작성된 메시지 표시 (`<br />` 태그 처리)
- ✅ 서브 헤더 사용 (sticky 지원)
- ✅ 보내기 버튼 (메시지 입력 시 활성화)
...
- ✅ 발신자 정보 표시 (3등분 레이아웃, space-between)
- ✅ 카드 공개 토글 (카드 UI ↔ 편지지 UI 전환)
- ✅ 카드 UI: 일러스트, 메시지 버블(absolute 하단 중앙), 닫는 메시지
- ✅ 편지지 UI: 헤더 이미지(민트 그린 배경, 상단 둥근 모서리), 본문(줄별 구분선), 닫는 메시지
